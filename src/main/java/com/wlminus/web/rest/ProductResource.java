...
/**
 * REST controller for managing {@link com.wlminus.domain.Product}.
 */
@RestController
@RequestMapping("/api")
public class ProductResource {
     ...
     */
    @GetMapping("/products")
    public ResponseEntity<List<Product>> getAllProducts(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false) String filter, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        if ("orderdesc-is-null".equals(filter)) {
            log.debug("REST request to get all Products where orderDesc is null");
            return new ResponseEntity<>(productService.findAllWhereOrderDescIsNull(),
                    HttpStatus.OK);
        }
